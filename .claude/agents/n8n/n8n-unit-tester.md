...
describe('fetchExternalData', () => {
  ...
  it('should handle API errors', async () => {
    ...
    await expect(fetchExternalData('https://api.example.com')).rejects.toThrow('API Error: 404');
  });
});
...
function calculateDiscount(order) {
  ...
  else if (customerTier === 'silver') discount += total * 0.1;
  else if (customerTier === 'bronze') discount += total * 0.05;

  // Promo code
  if (promoCode === 'SAVE20') discount += total * 0.2;

  return Math.min(discount, total * 0.4); // Max 40% discount
}
```
