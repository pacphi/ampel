...
async function processTaskLearning(jsonInput) {
  try {
    ...
    const hasSuccess = successIndicators.some((ind) =>
      agentOutput.toLowerCase().includes(ind.toLowerCase())
    );
    const hasFailure = failureIndicators.some((ind) =>
      agentOutput.toLowerCase().includes(ind.toLowerCase())
    );
    ...
    if (recentLearning) {
      ...
      try {
        ...
        if (meta.capturedBy !== 'PostToolUse-hook') {
          ...
          console.log(
            `📚 Learning: Agent ${agentType} already stored learning via MCP - skipping hook capture`
          );
          db.close();
          return;
        }
