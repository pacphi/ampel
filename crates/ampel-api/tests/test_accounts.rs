/// Integration tests for provider account handlers
///
/// Tests PAT account management including CRUD operations,
/// token validation, and default account selection.
mod common;
...
#[tokio::test]
async fn test_add_account_invalid_provider() {
    if TestDb::skip_if_sqlite() {
        return;
    }
...
async fn test_get_account_not_found() {
    ...
    let request = Request::builder()
        .method("GET")
        .uri(format!("/api/accounts/{}", fake_uuid))
        .header(header::AUTHORIZATION, format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
...
async fn test_validate_account_not_found() {
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/api/accounts/{}/validate", fake_uuid))
        .header(header::AUTHORIZATION, format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
...
#[tokio::test]
async fn test_set_default_account_not_found() {
    if TestDb::skip_if_sqlite() {
        return;
    }
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/api/accounts/{}/set-default", fake_uuid))
        .header(header::AUTHORIZATION, format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
